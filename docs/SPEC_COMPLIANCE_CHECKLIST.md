...
| **IA-U7** | Settings, support, electrician compliance embedded in Profile | ✅ Pass | ✅ Pass | ✅ Pass | ✅ Pass | Mobile: [`mobile/components/ProfileEssentials.tsx`](../mobile/components/ProfileEssentials.tsx) renders account + settings + support + logout; electrician compliance is inlined on Profile via [`ElectricianComplianceEmbedded`](../mobile/components/electrician/ElectricianCertificationScreen.tsx) from [`ElectricianProfileScreen.tsx`](../mobile/components/electrician/ElectricianProfileScreen.tsx) (no `(electrician)/compliance` tab route). Website: compliance embedded in `electrician/profile.tsx`. |
...
| **IA-U10** | Mobile & website same screens, order, data sources | ✅ Pass | 🟨 Partial | ✅ Pass | 🟨 Partial | Audits enforce mobile `mobile/app/(role)/…` shells + lazy map in [`website/src/App.tsx`](../website/src/App.tsx). **Electrician deferral:** shared registry [`stakeholderSections.ts`](../packages/shared/src/stakeholderSections.ts) keeps tab **id `jobs`** while `mobileRoute` targets `/(electrician)/projects` (Expo basename). Web still loads `./screens/stakeholders/electrician/jobs.tsx` (`jobs:` lazy until Codex aligns loaders + route to **`projects`**). **`packages/shared/scripts/audit-ui-compliance.cjs`** maps electrician mobile `projects` → web file/loader **`jobs`** via `electricianWebStakeholderFilename()`. APIs unchanged (`@emappa/api-client`). |
...
| R-5 | Building availability state (A0–A6) visible on Home | §3 | High | 🟨 | [`BuildingAvailabilityStatePill`](../mobile/components/shared/BuildingAvailabilityStatePill.tsx) | Shared primitive renders all 7 IA labels + palette; Resident Home not wired yet |
...
| R-7 | Capacity status display (cleared / queued / waitlisted / blocked) | §3, §6.2 | High | 🟨 | [`CapacityQueueStatusPill`](../mobile/components/shared/CapacityQueueStatusPill.tsx) | Shared primitive renders all 7 §6.2 labels + palette; Resident Home not wired yet |
...
| Tab | Spec requirement | Doc location | Current implementation | Status | Gap notes |
|-----|------------------|--------------|------------------------|--------|-----------|
| **Discover** | Airbnb-style project cards with distance, DRS, funding status, required crew size, scope, pay estimate, urgency | Scenario D §9 | `mobile/app/(electrician)/discover.tsx` → [`ElectricianDiscoverScreen.tsx`](../mobile/components/electrician/ElectricianDiscoverScreen.tsx): DRS roll-up hero + blocker rows + shallow “nearby sites (mock)” list + CTA to Projects; not a full Airbnb card grid yet. | 🟨 Partial | ~3 of 8 spec items; marketplace depth still shallow. |
| **Projects** (registry id `jobs`) | IA tab “Projects”: active-board + backlog/queue UX; Scenario D checklist/detail/sign-off surfaces | Scenario D §11–18; IA §Electrician tabs | [`ElectricianProjectsScreen.tsx`](../mobile/components/electrician/ElectricianProjectsScreen.tsx): segmented **Today / Queue** — `ElectricianHomeScreen` (“Today”) + [`ElectricianJobsInboxScreen`](../mobile/components/electrician/ElectricianJobsInboxScreen.tsx) (“Queue”). Missing: Scenario D staged/completed tabs, deadlines, full task board fidelity (P5). | 🟨 Partial | Segment merge done (P0.1.1); IA naming aligned (P0.1.5); queue screen promoted (P0.1.7). |
| **Wallet** | Job earnings, payout history, projected pipeline | Scenario D §22 | [`ElectricianEarningsScreen.tsx`](../mobile/components/electrician/ElectricianEarningsScreen.tsx) (71 lines): single estimate card (KES 84k) ✅. Missing: payout history list, projected pipeline. | 🟨 Partial | 1 of 3 spec items. |
| **Profile** | Account + **Settings** + **Support** + certification/compliance (embedded) + logout | Scenario D §28, IA §7 | [`ElectricianProfileScreen.tsx`](../mobile/components/electrician/ElectricianProfileScreen.tsx): `ElectricianTrustCard` ✅, **`ElectricianComplianceEmbedded`** (gates, field credential, scheduling state) ✅, `ProfileEssentials` (settings/support/logout) ✅. | ✅ Pass | All 5 spec items field-verified; IA-U7 met without a standalone compliance route. |

### 4.6 Financier (5 tabs)

| Tab | Spec requirement | Doc location | Current implementation | Status | Gap notes |
|-----|------------------|--------------|------------------------|--------|-----------|
| **Discover** | Airbnb-style project cards with DRS, capital raised/gap, stakeholder-as-a-service claims, risk badges, projected payback range, eligibility state | Scenario F §7 | `FinancierDealsScreen.tsx` (61L) + `FinancierDealDetailScreen.tsx` (38L): `BuildingSnapshotCard` + `MetricRail` + `DealPipelineCard` ✅, DRS callout ✅, capital raised/gap ✅. [`EligibilityBadge`](../mobile/components/shared/EligibilityBadge.tsx) primitive (5 IA states) — not wired on screen yet. Missing: SaaS claims, risk badges, payback range. | 🟨 Partial | 2 of 7 spec items. |
...
| **Energy generation** | E_gen vs E_sold, utilization, waste, battery use, grid fallback, asset ownership split, financier's economic exposure; projections pre-live, measured post-LBRS | Scenario F §13 | `mobile/components/financier/FinancierGenerationScreen.tsx` (71L): E_gen vs E_sold ✅ (StatusRail), utilization ✅, waste ✅, grid fallback ✅, projections pre-live ✅ (pilot banner), measured post-LBRS ✅. [`DataQualityBadge`](../mobile/components/shared/DataQualityBadge.tsx) primitive (5 IA states) — not wired on screen yet. Missing: explicit battery charge/discharge decomposition, asset ownership split, economic exposure breakdown. | 🟨 Partial | 5 of 9 spec items. |
...
- **2026-05-17** (v2.12) — **BUILD_PLAN P0.2.1 (`BuildingAvailabilityStatePill`)** — Added [`mobile/components/shared/BuildingAvailabilityStatePill.tsx`](../mobile/components/shared/BuildingAvailabilityStatePill.tsx): typed `A0`–`A6` states, IA Scenario A §3 labels, official-palette pill styling. R-5 partial (primitive only; Home adoption pending).
- **2026-05-17** (v2.11) — **BUILD_PLAN P0.1.7 (proposed-flow audit)** — Removed `mobile/components/proposed-flow/`. Promoted: [`ElectricianJobsInboxScreen`](../mobile/components/electrician/ElectricianJobsInboxScreen.tsx), [`ProviderAcceptTermsScreen`](../mobile/components/provider/ProviderAcceptTermsScreen.tsx), [`ProviderCommitCapacityScreen`](../mobile/components/provider/ProviderCommitCapacityScreen.tsx), [`ProviderQualifiedProjectsScreen`](../mobile/components/provider/ProviderQualifiedProjectsScreen.tsx), [`ProposedPageChrome`](../mobile/components/shared/ProposedPageChrome.tsx). Deleted obsolete: `FinancierTrancheReleaseScreen` (superseded by P0.1.3 embed), `ResidentVerifyPhoneScreen`, `ResidentJoinBuildingScreen` (zero importers; auth uses `verify-otp`).
- **2026-05-17** (v2.10) — **BUILD_PLAN P0.1.3 (financier tranche → payback embed)** — Removed hidden tab `mobile/app/(financier)/tranche-release.tsx`. Tranche/milestone release + payback scenario UX lives at [`mobile/app/(financier)/_embedded/payback-scenarios.tsx`](../mobile/app/(financier)/_embedded/payback-scenarios.tsx) via [`FinancierPaybackScenariosScreen`](../mobile/components/financier/FinancierPaybackScenariosScreen.tsx). [`RoleTabs`](../mobile/components/RoleTabs.tsx) hides `_embedded` instead of `tranche-release`; [`RoleDashboardScaffold`](../mobile/components/roles/RoleDashboardScaffold.tsx) deep links (`tranche`, `release`, `payback`, `scenario`) target the embed. Portfolio/Wallet tap-through to scenarios. **Web:** no `payback-scenarios` portal route yet — IA-U10 parity deferred to Codex.
- **2026-05-17** (v2.9) — **BUILD_PLAN P0.1.6 (owner → building-owner folder)** — Removed `mobile/components/owner/`; embedded BO routes import API-backed screens from [`BuildingOwnerScreens.tsx`](../mobile/components/building-owner/BuildingOwnerScreens.tsx). Kept [`BuildingOwnerShared.tsx`](../mobile/components/building-owner/BuildingOwnerShared.tsx) + [`BuildingOwnerAccountScreen.tsx`](../mobile/components/building-owner/BuildingOwnerAccountScreen.tsx) for hidden owner-account route. Deleted duplicate `OwnerHome`/`OwnerEarnings`/`OwnerListBuilding` scaffolds and all `proposed-flow/Owner*` copies.
- **2026-05-17** (v2.8) — **BUILD_PLAN P0.1.5 (installer → electrician folder)** — Renamed `mobile/components/installer/` → `mobile/components/electrician/`; `Installer*` symbols → `Electrician*` (shared scaffold in `ElectricianShared.tsx`). Deleted unused `InstallerChecklistScreen`, `InstallerJobDetailScreen`, `InstallerMaintenanceScreen` (P5 replacements). Renamed `proposed-flow/InstallerJobsInboxScreen` → `ElectricianJobsInboxScreen`. Updated `(electrician)/*` route imports + §4.5 checklist paths.
- **2026-05-16** (v2.7) — **BUILD_PLAN P0.1.2 (electrician compliance in Profile)** — Removed `mobile/app/(electrician)/compliance.tsx` (formerly a hidden Tabs route). Compliance UI lives in-profile via [`ElectricianComplianceEmbedded`](../mobile/components/electrician/ElectricianCertificationScreen.tsx) on [`ElectricianProfileScreen`](../mobile/components/electrician/ElectricianProfileScreen.tsx). Deep links from `ElectricianShared` action routing and [`RoleDashboardScaffold`](../mobile/components/roles/RoleDashboardScaffold.tsx) now target `/(electrician)/profile`. [`RoleTabs`](../mobile/components/RoleTabs.tsx): dropped electrician `compliance` hidden-tab entry + icon slot.
- **2026-05-16** (v2.6) — **BUILD_PLAN P0.1.1 (electrician Projects on mobile)** — Deleted `jobs-inbox.tsx` + `jobs.tsx` routes in favor of `mobile/app/(electrician)/projects.tsx` (segmented Today/Queue); discover no longer wrongly re-exported inbox. Shared `mobileRoute` `/(electrician)/projects`; **web portal + lazy map still keyed `jobs`** until Codex aligns IA-U10. [`audit-ui-compliance.cjs`](../packages/shared/scripts/audit-ui-compliance.cjs) electricians row uses mobile `projects` + web remap to `jobs` file + loader key.
- **2026-05-16** (v2.5) — Field-walk of the remaining 14 Provider/Electrician/Financier §4 rows. For each, the underlying component file was opened and rendered content compared item-by-item against the relevant scenario doc screen list.
  - **3 upgrades to ✅ Pass:**
    ...
    - §4.5 Electrician Profile (5/5 items: trust card, inlined compliance embedded block, settings, support, logout — `InstallerProfileScreen.tsx` + `InstallerComplianceEmbedded`)
