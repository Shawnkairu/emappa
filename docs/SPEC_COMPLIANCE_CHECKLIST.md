...
| **Projects** (registry id `jobs`) | IA tab “Projects”: active-board + backlog/queue UX; Scenario D checklist/detail/sign-off surfaces | Scenario D §11–18; IA §Electrician tabs | [`ElectricianProjectsScreen.tsx`](../mobile/components/electrician/ElectricianProjectsScreen.tsx): segmented **Today / Queue** — `ElectricianHomeScreen` (“Today”) + `ElectricianJobsInboxScreen` (“Queue”, `proposed-flow/`). Missing: Scenario D staged/completed tabs, deadlines, full task board fidelity (P5). | 🟨 Partial | Segment merge done (P0.1.1); IA naming aligned (P0.1.5). |
