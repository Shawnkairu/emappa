...
- **2026-05-16** (v2.7) — **BUILD_PLAN P0.1.2 (electrician compliance in Profile)** — Removed `mobile/app/(electrician)/compliance.tsx` (formerly a hidden Tabs route). Compliance UI lives in-profile via [`InstallerComplianceEmbedded`](../mobile/components/installer/InstallerCertificationScreen.tsx) on [`InstallerProfileScreen`](../mobile/components/installer/InstallerProfileScreen.tsx). Deep links from `InstallerShared` installer action routing and [`RoleDashboardScaffold`](../mobile/components/roles/RoleDashboardScaffold.tsx) now target `/(electrician)/profile`. [`RoleTabs`](../mobile/components/RoleTabs.tsx): dropped electrician `compliance` hidden-tab entry + icon slot.
